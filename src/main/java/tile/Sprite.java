...
public abstract class Sprite extends AbstractTile {
	...
	public LinkedList<Point> move(float dx, float dy) {
		// System.out.println("------------------------------------");
		// System.out.println("sou:"+GridMapRender.pxTolTile(x,
		// y)+" , tar:"+GridMapRender.pxTolTile(dx, dy));
	...
	private void moveTile(float dx, float dy, float nx, float ny) {
		...
		// boolean checkX = (velocityX > 0 && dx > nx)||(velocityX < 0
		// &&dx<nx+getWidth());
		// boolean checkY = (velocityY > 0 && dy > ny)||(velocityY < 0
		// &&dy<ny+getHeight());
		//
		// if(checkX || checkY){
	...
	protected void adjustFight() {
		...
		if (targetX == tileX || targetY == tileY) {
			...
			if (tileX < targetX) {
				currentAnim = tileY < targetY ? animations[ainmStatus][SOUTH_EAST]
						: animations[ainmStatus][NOTH_EAST];
			} else {
				currentAnim = tileY < targetY ? animations[ainmStatus][SOUTH_WEST]
						: animations[ainmStatus][NOTH_WEST];
			}
		}
	}
	...
	public void onArrive() {
		...
		// if(tileX<targetX){
		// currentAnim =
		// tileY<targetY?animations[0][SOUTH_EAST]:animations[0][NOTH_EAST];
		// }else{
		// currentAnim =
		// tileY<targetY?animations[0][SOUTH_WEST]:animations[0][NOTH_WEST];
		// }
		// }
		// }
	}
	...
	protected void notifyFightSource() {
		...
		for (int i = 0; i < fightSourceList.size(); ++i) {
			LinkedList<AStarNode> fightNodes = AStarSearch.findFightPath(
					gridMap, this);
	...
	protected void updateAnim() {
		...
		if (velocityX != 0 && velocityY == 0) {
			...
			newAnim = velocityX > 0 ? animations[ainmStatus][EAST]
					: animations[ainmStatus][WEST];
		}
		// 判断上下
		else if (velocityY != 0 && velocityX == 0) {

			newAnim = velocityY > 0 ? animations[ainmStatus][SOUTH]
					: animations[ainmStatus][NOTH];
		}
		// 判断四个斜角
			...
			if (velocityX > 0) {
				...
				newAnim = velocityY > 0 ? animations[ainmStatus][SOUTH_EAST]
						: animations[ainmStatus][NOTH_EAST];

			} else {

				newAnim = velocityY > 0 ? animations[ainmStatus][SOUTH_WEST]
						: animations[ainmStatus][NOTH_WEST];
	...
	public void draw(Graphics2D g, int offsetX, int offsetY) {
		...
		if (isSelected()) {
			...
			if (getType() != gridMap.getTileMapRender().getCurrentType()) {
				...
				g.drawArc(x + getWidth() / 4, y + getHeight() / 2,
						getWidth() / 2, getHeight() / 4, 0, 360);
			} else {
				g.setColor(Constant.GREEN);
				g.drawArc(x + getWidth() / 4, y + getHeight() / 2,
						getWidth() / 2, getHeight() / 4, 0, 360);
			}
			g.fillRect(x, y + getHeight() - 5,
					Math.round(getWidth() * (health)), 3);
	...
	public Tile clone(int x, int y, GridMap map) {
		...
		try {
			Sprite sprite = (Sprite) constructor.newInstance(new Object[] {
					ans, id });
...
/*
 * 数据结果： sou:java.awt.Point[x=1,y=3] , tar:java.awt.Point[x=1,y=4]
 * path:[java.awt.Point[x=1,y=4]] ------------------------------------
 * sou:java.awt.Point[x=1,y=3],tar:java.awt.Point[x=1,y=3]
 * path:[java.awt.Point[x=1,y=4]] ------------------------------------
 * 原因：因为是多个sprite先使用了findSpace查找空节点，先发现[x=1,y=3]已经有sprite 所以第二个被分到了x=1,y=4
 * ------------------------------------
 */
